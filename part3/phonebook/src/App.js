    ...
    if(existingUser) {
      if(window.confirm(`'${existingUser.name}' is already added to phonebook, replace the old number with a new one?`)){
        ...
        personsService
          ...
          .catch(error => {
            popErrMsg(`Failed to save '${updatedUser.name} (${updatedUser.number})': ${error.response.data.error}`);
            console.error(error);
          });
      } else {
        // 2.
      ...
      personsService
        ...
        .catch(error => {
          popErrMsg(`Failed to save '${newPerson.name} (${newPerson.number})': ${error.response.data.error}`);
          console.error(error);
        });
    }
  };
    ...
    personsService
      ...
      .catch(error => {
        if(error.response?.status === 404) {
          ...
          popErrMsg(`Failed to delete '${target.name}': ${error.response.data.error}`)
        }
        console.error(error);
      })
  };
